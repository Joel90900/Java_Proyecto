...
/**
 * API REST OBD2 (Bluetooth ELM327) → Spring Boot → MySQL.
 * Recibe las lecturas reales de la ECU, las guarda, calcula nivel/estado
 * y genera alertas cuando los valores presentan comportamientos anormales.
 */
@RestController
...
public class Obd2Controller {
    ...
    // ---------------------------------------------------------
    // GUARDAR LECTURA REAL (enviada desde la app vía Bluetooth)
    // ---------------------------------------------------------
    ...
    public Map<String, Object> guardarLectura(
        ...
        // Generar alerta si hay comportamiento anormal (nivel >= 40)
        if (nivel >= 40) {
            Alerta a = new Alerta();
    ...
    // ---------------------------------------------------------
    // GUARDAR CONFIGURACIÓN ELM327 (para reconexiones futuras)
    // ---------------------------------------------------------
...
// ---------------------------------------------------------
    // HISTORIAL DE LECTURAS DE UN VEHÍCULO
    // ---------------------------------------------------------
    ...
    // ---------------------------------------------------------
    // ÚLTIMA LECTURA REAL POR VEHÍCULO (pantalla en vivo)
    // ---------------------------------------------------------
    ...
    // ---------------------------------------------------------
    // HELPER: convertir valor real de sensor OBD2 a nivel porcentual
    // para representarlo igual que la web (OK/ADVERTENCIA/FALLA).
    // ---------------------------------------------------------
